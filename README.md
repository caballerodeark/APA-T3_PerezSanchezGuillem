...
def __mul__(self,other):
    ...
    if isinstance(other, Vector):  # Producto de Hadamard
    ...
    elif isinstance(other, (int, float)):  # Producto escalar
...
def __floordiv__(self, other):
    ...
    if isinstance(other, Vector):
        ...
        return Vector([scalar_projection * x for x in other.elements])  # v1^||
```
...
def __mod__(self, other):
    ...
    if isinstance(other, Vector):
        return Vector([a - b for a, b in zip(self.elements, (self // other).elements)])  # v1^⊥ = v1 - v1^||
```
